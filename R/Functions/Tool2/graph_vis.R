...
graph_vis <-function(df_MW_compiled,name,log_flag){
  ...
  tval <- sort(as.vector(sapply(seq(1,9), 
                                function(x) 
                                  x*10^seq(ceiling(log10(min(df_all$Concentration,na.rm=TRUE))),
                                           ceiling(log10(max(df_all$Concentration,na.rm=TRUE))))))) #generates a sequence of numbers in logarithmic divisions
