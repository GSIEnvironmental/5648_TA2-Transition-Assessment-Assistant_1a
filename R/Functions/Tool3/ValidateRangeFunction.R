ValidateRangeFunction<-function(error,results,input){
  ...
  validate(
    need((input$Year_Removed-input$Year_Started)>=10&(input$Year_Removed-input$Year_Started)<=100,
         "Time between Source Started year and Source Removed year outside model range (10-100 years) or Source Started year is later than the Source Removed year."),
    need(input$tortuosity_LK>=0.07&input$tortuosity_LK<=0.7,
         "Tortuosity outside model range (0.07-0.7)."),
    need((input$Year_Removed-input$Year_Started)-0.75*input$Retardation_HK*(input$X/(input$K*input$i/input$ne*60*60*24*365/100))>0,
         "Monitoring well is too far away from the source. Please see the help button."),
    ...
    need((input$X/(input$K*input$i/input$ne*60*60*24*365/100)>=0.4)&(input$X/(input$K*input$i/input$ne*60*60*24*365/100)<=11.9),
         paste("Travel time is ",toString(signif(input$X/(input$K*input$i/input$ne*60*60*24*365/100),2))," years which is outside model range (0.4-11.9 years).",sep='')),
