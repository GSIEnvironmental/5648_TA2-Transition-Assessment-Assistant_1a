...
HelpDoclist_Tool5<-function(){
  ...
  c(
    ...
    # select well to evaluate (#9)
    tags$div("The tool will project the concentration vs distance from this well.")
...
#question 4
  tags$div("Tool 3 provides a simple quantitative approach for estimating how complete source removal will 
  impact the remediation timeframe. It provides an estimate of the number of years it will take to reduce 
  the concentration in a plume monitoring well by 90%, 99%, or 99.9% after complete source removal has been 
  implemented. The Tool was developed by Dr. Bob Borden (Borden and Cha, 2021) and is based on the REMChlor-MD model.",
           tags$br(),
           tags$br(),
...
"References:",
tags$br(),
tags$li("Borden, R.C. and Cha, K.Y., 2021. Evaluating the impact of back diffusion on 
        groundwater cleanup time. Journal of Contaminant Hydrology, 243, p.103889")),
...
"),
  ...
  tags$div("At sites where the current plume footprint does not extend to the property boundary or other downgradient point of compliance, it is helpful to understand if there is area available for potential plume expansion. Such plume expansion could occur if active treatment measures are discontinued, such as shutting off the farthest downgradient extraction well for a pump and treat system. In these cases, the presence of a downgradient area that can be used for further attenuation of the plume is helpful. This is particularly true if active treatment was being used to provide plume containment since this area can be used to assess plume stability during a rebound test or after permanently shutting off pumping wells. If the plume takes additional time to stabilize, then this area serves as a plume assimilative capacity zone that allows concentrations to further decline prior to ultimately reaching the downgradient point of compliance."),
  tags$div("The primary goal of Tool 5 is to determine if attenuation rate is sufficient to ensure that the concentration cleanup goal will be achieved at the downgradient point of compliance distance from the well of concern after the end of active treatment. In other words, it determines if the assimilative capacity of the aquifer is enough to attenuate the contaminant mass discharge to acceptable levels.",
           tags$br(),
           "This forecast can be made with one or more of the rate constants that can be estimated using this tool, including pre-remediation data, lab-based data, or post-remediation data. Based on the type of rate constant that was selected, the tabs within Tool 5 contain graphs that show the projected concentration with distance during the post-remediation period from any well of interest. The concentration at the downgradient point of compliance is then compared to the user-specified cleanup goal."),
...
tags$li("Truex, M., Johnson, C., Macbeth, T., Becker, D., Lynch, K., Giaudrone, D., Frantz, A. and Lee, H., 2017. Performance assessment of pump‐and‐treat systems. Groundwater Monitoring & Remediation, 37(3), pp.28-44"
  ...
  tags$div("In some cases, changes in environmental conditions may influence the assimilative capacity of the aquifer and plume stability. Therefore, evaluating the appropriateness of MNA at a particular site should also consider the sustainability of conditions and processes at that site. Several approaches can be used to assess the sustainability of natural attenuation processes. This can include assessing long-term geochemical data (e.g., dissolved oxygen, oxidation-reduction potential) from wells located upgradient and within the plume to document that geochemical conditions that promote (or reflect) the desired reactions are not changing over time. Within the TA2 Tool, Tool 2 can be used to determine rate constants for subsets of the monitoring period, and this may help support a sustainability assessment. For example, the rate constant obtained for a recent monitoring period can be compared to the rate constant for the entire monitoring period (or an earlier monitoring period) to better understand if the attenuation rates are slowing down over time."),
...
"),
  tags$div("Tool 7 summarizes multiple Enhanced Attenuation (EA) approaches that can be evaluated on a site-specific basis"),
  tags$div("Tool 7b describes several EA approaches that can be applied in the source area to reduce contaminant loading from the unsaturated zone or in the saturated zone. In this case, the goal is to reduce the mass discharge from the source, such that the resulting groundwater concentration can then be naturally attenuated to an acceptable level within a shorter, reasonable timeframe."),
  tags$div("Tool 7b describes several EA approaches that can be applied in the source area to increase attenuation capacity. In this case, the goal is to increase the capacity of the aquifer to naturally attenuate the source, such that the cleanup goal for the site can be achieved within a shorter, reasonable timeframe."),
  tags$div("For sites that are being managed under many state regulatory programs or RCRA, MNA can often be used as part of a risk management strategy.  In these cases, the overall goal is to prevent a plume from exceeding a specified goal concentration at a downgradient point of compliance. Tool 1, Tool 2, Tool 5, and Tool 7 are useful in showing how MNA can be part of a treatment train approach to help achieve this goal."),
...
"),
  ...
  tags$div("Tool 7 summarizes multiple Enhanced Attenuation (EA) approaches that can be evaluated on a site-specific basis."),
  tags$div("These tools describe several EA approaches that can be applied in the source, plume, or discharge area(s) to reduce contaminant loading from the unsaturated zone or in the saturated zone.  The goal in this case is to reduce the mass discharge to a level that can be attenuated by the natural assimilative capacity of the aquifer by the time groundwater reaches the downgradient point of compliance."),
  tags$div("These tools describe several EA approaches that can be applied in the source, plume, or discharge area(s) to increase attenuation capacity.  The goal in this case is to increase the capacity of the aquifer above its natural capacity, such that the contaminant mass discharge is attenuated by the time groundwater reaches the downgradient point of compliance.")
